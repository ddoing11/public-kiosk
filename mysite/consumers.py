...
class KioskWebSocketConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_print_request(self, data):
        ...
        try:
            ...
            # [수정] 날짜 변환
            normalized_date = utils.normalize_date_input(issue_date_text)
            ...
            status, detail = await database_sync_to_async(utils.print_document)(
    ...
    async def process_voice_input(self, text):
        ...
        try:
            ...
            if current_step == 'date_selection':
                ...
                # [수정] 날짜 변환 실패 시
                if not utils.normalize_date_input(text): # 날짜 변환 실패 시 None 반환
                    # [수정] 새 헬퍼 함수 사용
